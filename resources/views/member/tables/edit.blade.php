      ...
      <div class="row">
        ...
        <div class="col-12">
          ...
          <div class="card">
            ...
            <!-- /.card-header -->
            <!-- form start -->
            <form method="POST" action="{{ route('member.tables.update', $table->id) }}">
              @csrf
              @method('PUT')
              <div class="card-body">
                <div class="form-group">
                  <label for="table_number">{{ __('Table Number') }}</label>
                  <input type="number" class="form-control @error('table_number') is-invalid @enderror" id="table_number" name="table_number" value="{{ $table->table_number }}" placeholder="{{ __('Enter Table Number') }}">
                </div>
                @error('table_number')
                ...
                <div class="form-group">
                  ...
                  <input type="number" class="form-control @error('guest_count') is-invalid @enderror" id="guest_count" name="guest_count" value="{{ $table->guest_count }}" placeholder="{{ __('Enter Guest Count') }}">
                </div>
                @error('guest_count')
                  <div class="text-sm text-red-400">{{ $message }}</div>
                @enderror
